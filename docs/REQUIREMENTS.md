...
| AF-ADMIN-009 | A Ready operator can configure, disable, and rotate the core Brave Search provider through an exact live-verified preview/apply flow. API keys are write-only OS-backed references; configuration versions and evidence hashes bind research approval, cache identity, and invocation so rotation or policy changes invalidate stale approvals without granting model network authority. | Post-R1 | Verified |
| AF-ADMIN-010 | A Ready operator can grant an agent the fixed-endpoint Brave Search capability through the complete versioned policy editor. A capable pinned model may propose only the bounded `search_web` contract; the durable run pauses at `ApprovalRequired`, displays canonical query/endpoint/risk/expiry evidence, executes or denies exactly once after operator approval, and resumes with a bounded citation result. The credential and unrestricted network authority never enter model context. | Post-R1 | Verified |
| AF-ADMIN-011 | A Ready operator can create, disable, and rotate a named HTTPS bearer profile for AI-generated API skills through a live-verified hash-bound preview/apply flow. The token is a write-only OS-backed reference; stored headers are non-secret and may use only server-expanded request/correlation templates. An agent may receive the generic read capability, but the model sees `http_api_get` only when the selected Active+granted immutable skill declares `tool:http-api.get`; every exact profile/path/query/response limit/resolved endpoint pauses at `ApprovalRequired` and executes once without exposing the token or permitting origin/path escape. | Post-R1 | Verified |
...
| AF-LEARN-007 | AI-generated skill proposals may declare an exact bounded set of existing read-only AgentForge tool IDs. Tool requirements are included in generation input/hash/evidence, immutable package manifests, replay comparison, and independent permission-diff evaluation; they grant no capability by themselves. A credential-free local Qwen acceptance gate must generate and evaluate a Microsoft Partner Center customer-discovery skill using only `tool:http-api.get`, with API credentials remaining outside model and package context. | Post-R1 | Verified |
